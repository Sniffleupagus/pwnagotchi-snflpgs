...
class Widget(object):
    def __init__(self, xy, color=0, click_url=None):
        self.xy = xy
        self.color = color
...
class LabeledValue(Widget):
    ...
    def draw(self, canvas, drawer):
        if self.label is None:
            ...
            pos = self.xy
            w_size = self.label_font.getbbox("W")
            lw = w_size[2] - w_size[0]
            drawer.text(pos, self.label, font=self.label_font, fill=self.color)
            drawer.text((pos[0] + self.label_spacing + lw * len(self.label), pos[1]), self.value, font=self.text_font, fill=self.color)
